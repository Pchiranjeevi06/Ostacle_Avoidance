...
class Robot:
    ...
    def avoid_abstacles(self,point_cloud,dt):
        closest_obs = None
        dist = np.inf   
...
class Graphics:
    ...
    def draw_sensors_data(self,point_cloud):
        for point in point_cloud:
            pygame.draw.circle(self.map,self.red,point,5,0)
...
class Ultrasonic:
    ...
    def sence_obstacles(self,x,y,heading):
        obstacles =[]
        x1,y1 =x ,y
        start_angle =heading - self.sensor_range[1]
        finish_angle = heading + self.sensor_range[1]
        for angle in np.linspace(start_angle,finish_angle,7,False):
